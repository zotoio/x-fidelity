...
export class ReportHistoryManager {
  ...
  async exportHistory(
    ...
    const exportPath = path.join(
      resultsDir,
      `xfi-history-export-${Date.now()}.${format}`
    );
  ...
  private async addToHistoryIndex(
    workspaceRoot: string,
  ...
  private generateComparison(
    ...
    const changes = {
      ...
      warningsChange:
        current.summary.warningCount - previous.summary.warningCount,
      newIssues: [] as Array<{ file: string; rule: string; message: string }>,
      resolvedIssues: [] as Array<{
        file: string;
        rule: string;
        message: string;
      }>,
    ...
    if (previous.result && current.result) {
      ...
      const prevFiles = new Set(
        previous.result.XFI_RESULT.issueDetails.map(d => d.filePath)
      );
      const currFiles = new Set(
        current.result.XFI_RESULT.issueDetails.map(d => d.filePath)
      );

      changes.changedFiles = [...new Set([...prevFiles, ...currFiles])].filter(
        file => {
          const prevFileIssues =
            previous.result!.XFI_RESULT.issueDetails.find(
              d => d.filePath === file
            )?.errors.length || 0;
          const currFileIssues =
            current.result!.XFI_RESULT.issueDetails.find(
              d => d.filePath === file
            )?.errors.length || 0;
  ...
  private extractIssueMap(
    result: ResultMetadata
  ): Map<string, { file: string; rule: string; message: string }> {
