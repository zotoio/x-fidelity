...
export class ProgressReporter {
  ...
  private updateProgress(message?: string): void {
    ...
    const displayMessage = message || 
      `${currentPhase.name}... ${this.phaseProgress.toFixed(0)}%`;
  ...
  getCurrentState(): ProgressState {
    ...
    const currentPhaseContribution = (currentPhase.weight * this.phaseProgress) / 100;
    const overallProgress = ((previousPhasesWeight + currentPhaseContribution) / this.totalWeight) * 100;
...
export class ProgressManager {
  private readonly analysisPhases: ProgressPhase[] = [
    { name: 'Initializing', weight: 10, description: 'Preparing analysis environment' },
    { name: 'Scanning Files', weight: 20, description: 'Discovering source files to analyze' },
    { name: 'Loading Plugins', weight: 10, description: 'Initializing analysis plugins' },
    { name: 'Running Analysis', weight: 45, description: 'Executing code quality checks' },
    { name: 'Processing Results', weight: 10, description: 'Converting analysis results' },
    { name: 'Finalizing', weight: 5, description: 'Completing analysis and cleanup' }
  ];

  async runWithProgress<T>(
    title: string,
    operation: (reporter: ProgressReporter, token: vscode.CancellationToken) => Promise<T>,
    ...
    return vscode.window.withProgress({
      ...
      try {
        ...
        progress.report({ message: `Analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}` });
        throw error;
      }
    });
  }
  ...
  createCustomPhases(phases: { name: string; weight: number; description?: string }[]): ProgressPhase[] {
