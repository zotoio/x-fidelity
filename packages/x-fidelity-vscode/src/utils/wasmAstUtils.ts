...
export async function initializeWasmTreeSitter(
  ...
  try {
    ...
    logger.info(
      `[X-Fidelity VSCode] Extension path: ${extensionContext.extensionPath}`
    );
    ...
    try {
      if (typeof ParserClass.init === 'function') {
        ...
        logger.info(
          '[X-Fidelity VSCode] Parser initialized successfully with Parser.init()'
        );
      } else if (typeof ParserClass === 'function') {
        // Some versions don't require explicit init - try direct instantiation
        logger.info(
          '[X-Fidelity VSCode] Parser.init not available, trying direct instantiation'
        );
        ...
        logger.warn(
          '[X-Fidelity VSCode] Unknown Parser class structure - attempting graceful degradation'
        );
      }
    } catch (initError) {
      logger.warn(
        `[X-Fidelity VSCode] Parser initialization method failed: ${initError} - attempting fallback`
      );
