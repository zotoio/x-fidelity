...
export class DefaultDetectionService {
  ...
  private async detectNodeProject(): Promise<ProjectDetection | null> {
    ...
    try {
      ...
      // TypeScript projects
      if (dependencies.typescript || await this.fileExists(path.join(this.workspaceRoot, 'tsconfig.json'))) {
        indicators.push('TypeScript');
        confidence += 10;
      }
      
      // Test frameworks
      if (packageJson.scripts?.test || dependencies.jest || dependencies.mocha) {
  ...
  private async detectJavaProject(): Promise<ProjectDetection | null> {
    ...
    if (await this.fileExists(pomPath)) {
      ...
      try {
        ...
        if (pomContent.includes('spring-cloud') || pomContent.includes('eureka')) {
    ...
    if (await this.fileExists(gradlePath) || await this.fileExists(gradleKtsPath)) {
      indicators.push('Gradle build');
      confidence += 40;
    }
    
    // Check for Spring Boot application properties
    const applicationProps = path.join(this.workspaceRoot, 'src/main/resources/application.properties');
    const applicationYml = path.join(this.workspaceRoot, 'src/main/resources/application.yml');
    if (await this.fileExists(applicationProps) || await this.fileExists(applicationYml)) {
  ...
  private async detectPythonProject(): Promise<ProjectDetection | null> {
    ...
    if (await this.fileExists(path.join(this.workspaceRoot, 'requirements.txt'))) {
      indicators.push('requirements.txt');
      confidence += 30;
    }
    
    // Check for pyproject.toml
    if (await this.fileExists(path.join(this.workspaceRoot, 'pyproject.toml'))) {
  ...
  private async detectDotNetProject(): Promise<ProjectDetection | null> {
    ...
    if (await this.fileExists(path.join(this.workspaceRoot, 'appsettings.json'))) {
