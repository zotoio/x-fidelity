...
export class ScreenshotHelper {
  private static screenshotDir = path.resolve(process.cwd(), 'test-screenshots');
   ...
   */
  static async captureScreen(testName: string, options: ScreenshotOptions = {}): Promise<string> {
    if (!this.isScreenshotEnabled()) {return '';}
   ...
   */
  static async captureVSCodeWindow(testName: string, options: ScreenshotOptions = {}): Promise<string> {
    if (!this.isScreenshotEnabled()) {return '';}
    ...
    return this.captureScreen(testName, { ...options, description: 'vscode-window' });
  }
  ...
  static async captureAfterCommand(
    ...
    try {
      ...
      return this.captureScreen(testName, { ...options, description: 'command-failed' });
    }
  }
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      if (step.command) {
        try {
          ...
          console.warn(`⚠️ Command failed in workflow: ${step.command} - ${error}`);
        }
      }
  ...
  private static isScreenshotEnabled(): boolean {
    return process.env.SCREENSHOTS === 'true' && 
   ...
   */
  private static async updateScreenshotIndex(filename: string, testName: string, description: string): Promise<void> {
    try {
      const indexFile = path.join(this.screenshotDir, 'index.html');
