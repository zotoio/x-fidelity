...
export class AnalysisManager implements vscode.Disposable {
  ...
  private readonly onAnalysisStateChanged =
    new vscode.EventEmitter<AnalysisState>();
  ...
  async runAnalysis(options?: {
    ...
    try {
      ...
      const cached = await this.cacheManager.getCachedResult(
        analysisTargetPath,
      ...
      this.performPostAnalysisOperations(
        analysisTargetPath,
        analysisResult,
        config,
        operationId
      ...
      this.logger.debug('Analysis cleanup completed', {
        operationId,
        finalTime
      });
    }
  ...
  startPeriodicAnalysis(): void {
    ...
    if (config.runInterval > 0) {
      ...
      if (actualInterval !== config.runInterval) {
        this.logger.warn(
          `Periodic analysis interval increased from ${config.runInterval}s to ${actualInterval}s to prevent performance issues`
        );
      }
      ...
      this.periodicTimer = setInterval(() => {
        if (!this.isAnalyzing) {
          ...
          this.logger.debug(
            'Skipping periodic analysis - analysis already in progress'
          );
        }
      }, actualInterval * 1000);
  ...
  private async performPostAnalysisOperations(
    ...
    try {
      ...
      if (config.generateReports) {
        ...
        await this.reportManager.generateReports(
  ...
  private async performAnalysisWithProgress(
    ...
    return (
      (await this.progressManager.runWithProgress(
        'X-Fidelity Analysis',
        async (reporter: ProgressReporter, token: vscode.CancellationToken) => {
          // Phase 1: Initializing
          reporter.updatePhaseProgress(
            50,
          ...
          const configServer =
            config.configServer &&
            config.configServer.trim() &&
            (config.configServer.startsWith('http://') ||
              config.configServer.startsWith('https://'))
              ? config.configServer
              : undefined;
          const localConfigPath =
            this.configManager.getResolvedLocalConfigPath();
      ...
      if (diagnostics.length > 0) {
        ...
        if (detail.filePath === 'REPO_GLOBAL_CHECK') {
          ...
          if (workspacePath && detail.filePath.startsWith(workspacePath)) {
            ...
            if (
              analysisTargetPath &&
              detail.filePath.startsWith(analysisTargetPath)
            ) {
  ...
  private createDiagnostic(error: any): vscode.Diagnostic {
    const lineInfo = this.parseLineColumnInfo(
        ...
        range: error.details.range
          ? {
              start: {
                ...
                line:
                  error.details.range.end?.line ||
                  error.details.range.start?.line ||
                  1,
                column:
                  error.details.range.end?.column ||
                  error.details.range.start?.column ||
                  1
              }
            }
          : undefined,
