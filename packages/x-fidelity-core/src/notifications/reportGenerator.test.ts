...
describe('ReportGenerator', () => {
    ...
    describe('Package Size Section', () => {
        describe('generatePackageSizeSection', () => {
            it('should return empty string when no package size data is available', () => {
                const result = (generator as any).generatePackageSizeSection();
                expect(result).toBe('');
            });

            it('should generate section when package size data is in factResults', () => {
                    ...
                    XFI_RESULT: {
                        ...
                        factResults: {
                            packageSize: {
                                packages: [
                                    {
                                        name: 'x-fidelity-core',
                                        path: 'packages/x-fidelity-core',
                                        totalSize: 557400,
                                        sourceSize: 245300,
                                        buildSize: 312100,
                                        sourceBreakdown: { '.ts': 200000, '.json': 30000 },
                                        buildBreakdown: { '.js': 300000, '.d.ts': 12100 },
                                        exceedsWarning: false,
                                        exceedsFatality: false
                                    },
                                    {
                                        name: 'x-fidelity-plugins',
                                        path: 'packages/x-fidelity-plugins',
                                        totalSize: 391200,
                                        sourceSize: 189700,
                                        buildSize: 201500,
                                        sourceBreakdown: { '.ts': 180000, '.json': 9700 },
                                        buildBreakdown: { '.js': 190000, '.d.ts': 11500 },
                                        exceedsWarning: false,
                                        exceedsFatality: false
                                    }
                                ],
                ...
                const gen = new ReportGenerator(packageSizeData);
                const result = (gen as any).generatePackageSizeSection();
                
                expect(result).toContain('## Package Size Analysis');
                expect(result).toContain('**2 packages**');
                expect(result).toContain('926.37 KB');  // 948600 bytes formatted
                expect(result).toContain('yarn');
                expect(result).toContain('(monorepo)');
                expect(result).toContain('### Size Distribution');
                expect(result).toContain('```mermaid');
                expect(result).toContain('pie');
                expect(result).toContain('x-fidelity-core');
                expect(result).toContain('x-fidelity-plugins');
                expect(result).toContain('### Package Size Details');
                expect(result).toContain('| Package | Source | Build | Total | Status |');
                expect(result).toContain('✅ OK');
            });

            it('should generate section when package size data is in issue details', () => {
                const packageSizeIssueData = {
                    ...mockData,
                    XFI_RESULT: {
                        ...mockData.XFI_RESULT,
                        issueDetails: [
                            {
                                ...
                                errors: [
                                    {
                                        ruleFailure: 'packageSize-threshold-global',
                                        level: 'warning',
                                        details: {
                                            message: 'Package size exceeds threshold',
                                            details: {
                                                packages: [
                                                    {
                                                        name: 'large-package',
                                                        path: 'packages/large-package',
                                                        totalSize: 5242880, // 5 MB
                                                        sourceSize: 2097152, // 2 MB
                                                        buildSize: 3145728, // 3 MB
                                                        sourceBreakdown: { '.ts': 2000000, '.json': 97152 },
                                                        buildBreakdown: { '.js': 3000000, '.d.ts': 145728 },
                                                        exceedsWarning: true,
                                                        exceedsFatality: false,
                                                        warningThreshold: 4194304 // 4 MB
                                                    }
                                                ],
                                                totalSize: 5242880,
                                                workspaceType: 'npm',
                ...
                const gen = new ReportGenerator(packageSizeIssueData);
                const result = (gen as any).generatePackageSizeSection();
                ...
                expect(result).toContain('(single package)');
                expect(result).toContain('large-package');
                expect(result).toContain('⚠️ Warning');
            });

            it('should show fatality status for packages over limit', () => {
                const fatalityData = {
                    ...mockData,
                    XFI_RESULT: {
                        ...mockData.XFI_RESULT,
                                ...
                                packages: [
                                    {
                                        ...
                                        warningThreshold: 10485760,
                                        fatalityThreshold: 52428800
                                    }
                                ],
                                totalSize: 104857600,
                                workspaceType: 'pnpm',
                ...
                const gen = new ReportGenerator(fatalityData);
                const result = (gen as any).generatePackageSizeSection();
                
                expect(result).toContain('🔥 **OVER LIMIT**');
            });

            it('should include file type breakdown when available', () => {
                const breakdownData = {
                    ...mockData,
                    XFI_RESULT: {
                        ...mockData.XFI_RESULT,
                                ...
                                packages: [
                                    {
                                        ...
                                        sourceSize: 60000,
                                        buildSize: 40000,
                                        sourceBreakdown: { '.ts': 50000, '.json': 8000, '.md': 2000 },
                                        buildBreakdown: { '.js': 35000, '.d.ts': 5000 },
                                        exceedsWarning: false,
                                        exceedsFatality: false
                                    }
                                ],
                ...
                const gen = new ReportGenerator(breakdownData);
                const result = (gen as any).generatePackageSizeSection();
                
                expect(result).toContain('### File Type Breakdown');
                expect(result).toContain('| Package | Top File Types |');
            ...
            it('should sort packages by size descending', () => {
                ...
                const gen = new ReportGenerator(multiPackageData);
                const result = (gen as any).generatePackageSizeSection();
                
                // large-package should appear before medium-package which should appear before small-package
                const largeIndex = result.indexOf('large-package');
                const mediumIndex = result.indexOf('medium-package');
                const smallIndex = result.indexOf('small-package');
        ...
        describe('generatePackageSizeChart', () => {
            it('should return empty string for empty packages array', () => {
                const result = (generator as any).generatePackageSizeChart([]);
                expect(result).toBe('');
            });

            it('should generate valid mermaid pie chart', () => {
                const packages = [
                    { name: 'package-a', totalSize: 100000 },
                    { name: 'package-b', totalSize: 50000 }
                ];
                const result = (generator as any).generatePackageSizeChart(packages);
                
                expect(result).toContain('### Size Distribution');
                expect(result).toContain('```mermaid');
                expect(result).toContain('pie');
                expect(result).toContain('title Package Size Distribution');
                expect(result).toContain('"package-a" : 100000');
                expect(result).toContain('"package-b" : 50000');
                expect(result).toContain('```');
            });

            it('should limit to top 8 packages', () => {
                const packages = Array.from({ length: 12 }, (_, i) => ({
                    name: `package-${i}`,
                    totalSize: 100000 - (i * 1000)
                }));
                const result = (generator as any).generatePackageSizeChart(packages);
                
                expect(result).toContain('package-0');
                expect(result).toContain('package-7');
                expect(result).not.toContain('package-8');
                expect(result).not.toContain('package-11');
            });
        });

        describe('getPackageSizeStatus', () => {
            it('should return OK for packages within limits', () => {
                const pkg = { exceedsWarning: false, exceedsFatality: false };
                const result = (generator as any).getPackageSizeStatus(pkg);
                expect(result).toBe('✅ OK');
            });

            it('should return Warning for packages exceeding warning threshold', () => {
                const pkg = { exceedsWarning: true, exceedsFatality: false };
                const result = (generator as any).getPackageSizeStatus(pkg);
                expect(result).toBe('⚠️ Warning');
            });

            it('should return OVER LIMIT for packages exceeding fatality threshold', () => {
                const pkg = { exceedsWarning: true, exceedsFatality: true };
                const result = (generator as any).getPackageSizeStatus(pkg);
                expect(result).toBe('🔥 **OVER LIMIT**');
            });
        });

        describe('generateFileTypeBreakdownSection', () => {
            it('should generate file type breakdown table', () => {
                const packages = [
                    {
                        name: 'test-package',
                        sourceBreakdown: { '.ts': 50000, '.json': 8000 },
                        buildBreakdown: { '.js': 35000, '.d.ts': 5000 }
                    }
                ];
                const result = (generator as any).generateFileTypeBreakdownSection(packages);
            ...
            it('should show N/A for packages without breakdowns', () => {
                const packages = [
                    {
                        name: 'empty-package',
                        sourceBreakdown: {},
                ...
                expect(result).toContain('empty-package');
                expect(result).toContain('N/A');
            });

            it('should combine source and build breakdowns', () => {
                const packages = [
                    {
                        name: 'combined-package',
                        sourceBreakdown: { '.ts': 50000 },
                        buildBreakdown: { '.ts': 30000, '.js': 40000 } // .ts appears in both
                    }
                ];
                const result = (generator as any).generateFileTypeBreakdownSection(packages);
                
                // .ts should be combined (50000 + 30000 = 80000) and be the largest
                expect(result).toContain('.ts:');
                expect(result).toContain('78.13 KB'); // 80000 bytes formatted
            });

            it('should limit to top 3 file types', () => {
                const packages = [
                    {
                        name: 'many-types-package',
                        sourceBreakdown: { 
                            '.ts': 50000, 
                            '.json': 40000, 
                            '.md': 30000, 
        ...
        describe('extractPackageSizeData', () => {
            it('should return null when no package size data exists', () => {
                const result = (generator as any).extractPackageSizeData();
                expect(result).toBeNull();
            });

            it('should extract data from factResults.packageSize', () => {
                const factData = {
                    ...mockData,
                    XFI_RESULT: {
                        ...mockData.XFI_RESULT,
                        factResults: {
                            packageSize: {
                                packages: [{ name: 'test', path: 'test', totalSize: 1000, sourceSize: 500, buildSize: 500, sourceBreakdown: {}, buildBreakdown: {}, exceedsWarning: false, exceedsFatality: false }],
                                totalSize: 1000,
                                workspaceType: 'yarn',
                                isMonorepo: false,
                ...
                const gen = new ReportGenerator(factData);
                const result = (gen as any).extractPackageSizeData();
                
                expect(result).not.toBeNull();
                expect(result.packages).toHaveLength(1);
                expect(result.packages[0].name).toBe('test');
            });

            it('should extract data from issue details with packageSize rule', () => {
                const issueData = {
                    ...mockData,
                    XFI_RESULT: {
                        ...mockData.XFI_RESULT,
                        issueDetails: [
                            {
                                ...
                                errors: [
                                    {
                                        ruleFailure: 'packageSize-check-global',
                                        level: 'warning',
                                        details: {
                                            message: 'Package size issue',
                                            details: {
                                                packages: [{ name: 'from-issue', path: 'test', totalSize: 2000, sourceSize: 1000, buildSize: 1000, sourceBreakdown: {}, buildBreakdown: {}, exceedsWarning: false, exceedsFatality: false }],
                                                totalSize: 2000,
                                                workspaceType: 'npm',
                                                isMonorepo: true,
                                                analyzedAt: '2024-01-15',
                ...
                const gen = new ReportGenerator(issueData);
                const result = (gen as any).extractPackageSizeData();
                
                expect(result).not.toBeNull();
                expect(result.packages[0].name).toBe('from-issue');
            });

            it('should handle package-size rule pattern with hyphen', () => {
                const issueData = {
                    ...mockData,
                    XFI_RESULT: {
                        ...mockData.XFI_RESULT,
                        issueDetails: [
                            {
                                ...
                                errors: [
                                    {
                                        ruleFailure: 'package-size-threshold-global',
                                        level: 'fatality',
                                        details: {
                                            message: 'Package too large',
                                            details: {
                                                packages: [{ name: 'hyphen-rule', path: 'test', totalSize: 3000, sourceSize: 1500, buildSize: 1500, sourceBreakdown: {}, buildBreakdown: {}, exceedsWarning: true, exceedsFatality: true }],
                                                totalSize: 3000,
                                                workspaceType: 'pnpm',
                                                isMonorepo: false,
                                                analyzedAt: '2024-01-15',
                ...
                expect(result).not.toBeNull();
                expect(result.packages[0].name).toBe('hyphen-rule');
            });
        });

        describe('isPackageSizeResult', () => {
            it('should return true for valid package size result', () => {
                const validResult = {
                    packages: [],
                    totalSize: 0,
                    workspaceType: 'yarn',
                ...
                const result = (generator as any).isPackageSizeResult(validResult);
                expect(result).toBe(true);
            });

            it('should return falsy for null', () => {
                const result = (generator as any).isPackageSizeResult(null);
                expect(result).toBeFalsy();
            });

            it('should return falsy for undefined', () => {
                const result = (generator as any).isPackageSizeResult(undefined);
                expect(result).toBeFalsy();
            });

            it('should return false for non-object', () => {
                const result = (generator as any).isPackageSizeResult('string');
                expect(result).toBe(false);
            });

            it('should return false for missing packages array', () => {
                const invalid = { totalSize: 0, isMonorepo: true };
                const result = (generator as any).isPackageSizeResult(invalid);
                expect(result).toBe(false);
            });

            it('should return false for non-array packages', () => {
                const invalid = { packages: 'not-array', totalSize: 0, isMonorepo: true };
                const result = (generator as any).isPackageSizeResult(invalid);
                expect(result).toBe(false);
            });

            it('should return false for missing totalSize', () => {
                const invalid = { packages: [], isMonorepo: true };
                const result = (generator as any).isPackageSizeResult(invalid);
                expect(result).toBe(false);
            });

            it('should return false for missing isMonorepo', () => {
                const invalid = { packages: [], totalSize: 0 };
                const result = (generator as any).isPackageSizeResult(invalid);
                expect(result).toBe(false);
            });
        });

        describe('formatBytesForReport', () => {
            it('should format 0 bytes', () => {
                const result = (generator as any).formatBytesForReport(0);
                expect(result).toBe('0 Bytes');
            });

            it('should format bytes', () => {
                const result = (generator as any).formatBytesForReport(500);
                expect(result).toBe('500 Bytes');
            });

            it('should format kilobytes', () => {
                const result = (generator as any).formatBytesForReport(1024);
                expect(result).toBe('1 KB');
            });

            it('should format kilobytes with decimals', () => {
                const result = (generator as any).formatBytesForReport(1536);
                expect(result).toBe('1.5 KB');
            });

            it('should format megabytes', () => {
                const result = (generator as any).formatBytesForReport(1048576);
                expect(result).toBe('1 MB');
            });

            it('should format megabytes with decimals', () => {
                const result = (generator as any).formatBytesForReport(2621440);
                expect(result).toBe('2.5 MB');
            });

            it('should format gigabytes', () => {
                const result = (generator as any).formatBytesForReport(1073741824);
            ...
            it('should handle fractional bytes', () => {
                const result = (generator as any).formatBytesForReport(0.5);
                expect(result).toBe('0.5 Bytes');
            });
        });
    });
});